...
private:
    ...
    static void computeSuffixes(const std::string& pattern, std::vector<int>& suff);
    static void buildGoodSuffTable(const std::string& pattern, std::vector<int>& goodSuff);

public:
    static std::vector<Match> searchBoyerMooreHorspool(const std::string& text, const std::string& pattern);
